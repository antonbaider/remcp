...
const AI_TOOL_DESCRIPTIONS = Object.freeze({
  ...
  list_windows: 'Use this to discover top-level desktop windows or obtain a stable window id/PID/bounds before window_action, screenshot_region or targeted input. Prefer it over screenshot-based window guessing.',
  ...
  open_path: 'Use this to open a permitted file or directory in its normal desktop application. Prefer it over shell-specific open/start/xdg-open commands because ReMCP applies filesystem confinement.',
  ...
  environment: 'Use this for platform/runtime/path/shell facts and only opt into sanitized environment variables when the task truly needs them. Secret-looking values remain masked.',
  ...
  power_action: 'Use this only when the user explicitly asks to lock, sleep, restart or shut down the computer. It is intentionally guarded by device command policy because restart/shutdown can interrupt work.',
  ...
  read_document: 'Use this to read structured content from PDF, DOCX, XLSX, TXT, Markdown, CSV, JSON or XML without opening a desktop app. Prefer it over shell extraction or launching Office when the goal is document content.',
