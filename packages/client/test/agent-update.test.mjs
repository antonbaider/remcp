...
test('the agent updates the client, the runtime, or both', () => {
  ...
  assert.deepEqual(
    updateDecision({ advertised: advertised('@remcp/remcp@0.2.7', '@remcp/runtime@0.2.7'), cliVersion: '0.2.7', runtimeVersion: '0.2.7' }),
    { needed: false, target: '@remcp/remcp@0.2.7', runtime: '@remcp/runtime@0.2.7', reason: 'current' },
  );
  // Newer client.
  assert.equal(updateDecision({ advertised: advertised('@remcp/remcp@0.2.8', '@remcp/runtime@0.2.8'), cliVersion: '0.2.7', runtimeVersion: '0.2.7' }).reason, 'client');
  ...
  const runtimeOnly = updateDecision({ advertised: advertised('@remcp/remcp@0.2.7', '@remcp/runtime@0.2.7'), cliVersion: '0.2.7', runtimeVersion: '0.2.6' });
  ...
  assert.equal(updateDecision({ advertised: advertised('@remcp/remcp@0.2.7', '@remcp/runtime@0.2.7'), cliVersion: '0.2.7', runtimeVersion: 'unknown' }).needed, false);
  // A prerelease install moves forward to the release of the same version.
  assert.equal(updateDecision({ advertised: advertised('@remcp/remcp@0.3.0', '@remcp/runtime@0.3.0'), cliVersion: '0.3.0-beta.2', runtimeVersion: '0.3.0-beta.2' }).reason, 'client');
  // An older server must not downgrade a newer machine.
  assert.equal(updateDecision({ advertised: advertised('@remcp/remcp@0.2.6', '@remcp/runtime@0.2.6'), cliVersion: '0.2.7', runtimeVersion: '0.2.7' }).needed, false);
});
...
test('only a plain version of the configured runtime package is ever installed', async () => {
  ...
  assert.equal(decision.runtime, '', 'the untrusted spec is dropped');
});
...
test('a device whose runtime never started can repair itself', () => {
  ...
  const broken = updateDecision({ advertised, cliVersion: '0.2.9', runtimeVersion: 'unknown', runtimePackageName: '@remcp/runtime', runtimeDown: true });
  ...
  assert.equal(updateDecision({ advertised, cliVersion: '0.2.9', runtimeVersion: '0.2.10', runtimePackageName: '@remcp/runtime', runtimeDown: false }).needed, false, 'already on the advertised runtime');
  assert.equal(updateDecision({ advertised, cliVersion: '0.2.9', runtimeVersion: '0.2.9', runtimePackageName: '@remcp/runtime', runtimeDown: false }).needed, true, 'a newer advertised runtime is installed');
  // A running runtime is never reinstalled just because the agent is unsure.
  assert.equal(updateDecision({ advertised, cliVersion: '0.2.9', runtimeVersion: 'unknown', runtimePackageName: '@remcp/runtime', runtimeDown: false }).needed, false);
});
