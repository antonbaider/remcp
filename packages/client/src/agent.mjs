...
export async function runAgent(options) {
  ...
  if (!telemetryEnabled) runtimeEnv.REMCP_RUNTIME_DISABLE_TELEMETRY = '1';
  const mcp = new Client({ name: 'remcp-agent', version: VERSION });
  const transport = new StdioClientTransport({ command: process.execPath, args: [runtimeEntry], env: runtimeEnv });
  await mcp.connect(transport);
  runtimeVersion = mcp.getServerVersion()?.version || 'unknown';
  ...
  // The runtime emits usage metrics as an MCP notification. They are forwarded over the
  // authenticated WebSocket the agent already holds, so no new endpoint exists and the
  // runtime itself still opens no sockets.
  ...
  function flushTelemetry() {
    ...
    const delivered = send({ type: 'telemetry', runtimeVersion, agentVersion: VERSION, events: batch });
    if (!delivered) telemetryQueue.unshift(...batch);
  }
  ...
  async function respond(ws, message) {
    ...
    try {
      ...
      else if (message.method === 'ping') result = { ok: true, hostname: os.hostname(), platform: process.platform, arch: process.arch, uptimeSeconds: Math.floor(os.uptime()), agentVersion: VERSION, runtimeVersion };
  ...
  function connect() {
    ...
    ws.on('open', () => {
      ...
      send({ type: 'metrics', metrics: deviceMetrics({ reconnects, pendingRequests, runtimeVersion }) });
      reportInstallOnce();
      flushTelemetry();
    });
    ws.on('message', raw => {
  ...
  telemetryTimer = setInterval(() => {
    if (activeSocket?.readyState === 1) {
      send({ type: 'metrics', metrics: deviceMetrics({ reconnects, pendingRequests, runtimeVersion, queueDepth: telemetryQueue.length }) });
      flushTelemetry();
    }
  }, METRICS_INTERVAL_MS);
  ...
  connect();
  return { stop, runtimeVersion: () => runtimeVersion };
}
